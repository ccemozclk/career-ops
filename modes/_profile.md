...
<!-- ============================================================
     ...
     Cem Özçelik — Senior Data Scientist / ML Engineer / Data Analyst
     Istanbul, Turkey | Open to remote, hybrid, EU relocation
     ============================================================ -->
...
| **Senior Data Analyst** | Advanced SQL, EDA, supply chain analytics, statistical analysis, BI | Someone who bridges raw data and strategic decisions with rigorous analysis |
...
| **Amazon AI Search Engine (Semantic Search + Vector DB)** | github.com/ccemozclk/amazon-ai-search-engine | E-commerce, search, NLP, recommendation, vector databases, microservices/MLOps |
| **Bidirectional LSTM Sentiment (Custom PyTorch Architecture)** | github.com/ccemozclk/Sentiment-LSTM-PyTorch | NLP, deep learning, sequential modeling, custom architecture design |
| **Spark Recommendation Engine (RFM + K-Means + ALS)** | github.com/ccemozclk/Spark-Recommendation-Engine | Big data, distributed ML, customer segmentation, marketing ML, e-commerce |


## Your Archetype Detection Override

When evaluating a JD, map to Cem's archetypes:
...
| Forecasting / time-series / demand planning | Rossmann Forecasting → Credit Risk → Spark Recommendation (segmentation as planning input) |
| MLOps / DevOps / deployment / infrastructure | Credit Risk → Rossmann → Amazon AI Search Engine |
...
**Turkey (local market):**
- Senior DS / ML Engineer: 100,000–200,000 TL/month
- Senior Data Analyst: 80,000–150,000 TL/month

**EU remote / relocation:**
- Senior DS / ML Engineer: 55,000–90,000 EUR/year
- Senior Data Analyst: 45,000–70,000 EUR/year

**US remote:**
- Senior DS / ML Engineer: 130,000–180,000 USD/year (base) + equity
- Strong companies (FAANG-tier, top fintech): 160,000–220,000 USD base

**Contractor (EU, outside payroll):**
- Typical 25–40% premium over employee base

Use WebSearch for current market data (Glassdoor, Levels.fyi, LinkedIn Salary, local Turkey surveys by Universum or Paylab).

## Your Negotiation Scripts

**Salary expectations (generic):**
> "For Senior Data Scientist roles in this market I'm targeting [RANGE]. I'm flexible on structure depending on the total package and the scope of the role."

**Geographic discount pushback:**
> "The value I deliver — predictive models reducing machine downtime by 15%, ML pipelines sustaining 8-hour weekly overtime reductions — doesn't change based on my postal code. I'd want the role priced on output, not location."

**When offered below target:**
> "I'm comparing against opportunities in the [higher range]. I'm drawn to [company] because of [specific reason]. Is there flexibility to reach [target] or structure it differently?"

**On Industrial Engineering background (if questioned as non-traditional):**
> "Industrial Engineering gave me something most DS candidates don't have out of the box: a native instinct for cost-function design. Before I touch model architecture I ask what it costs when the model is wrong in each direction. That's shaped every high-stakes project I've delivered."

## Your Location Policy

**Open policy — evaluate everything, let the score decide:**
- Istanbul on-site: full score on location if role is strong
- Remote with EU/CET overlap (UTC-1 to UTC+3): full score — preferred
- Remote with EST overlap (UTC-5 to UTC+3): full score if mornings-only required
- Remote with PST/strict US hours: 2.5 — possible but burnout risk, only for exceptional roles
- Remote APAC: 1.5 — sleep cycle inversion, avoid
- Hybrid in Istanbul: full score
- EU hybrid (requiring relocation): score 3.5 on location, upgrade if relocation package is confirmed
- EU on-site (no remote, no relocation support): score 2.0 on location
- Only score 1.0 if JD says "must be on-site 5 days/week, no exceptions, no relocation"

**In forms:**
- Specify "CET +1h overlap comfortable (UTC+3 Istanbul)" for EU remote roles
- For US remote: "EST overlap available mornings (UTC+3)" — be specific

**Relocation:**
- Open to EU relocation for the right role + package
- Mention in cover letters for EU on-site roles: "open to relocation, familiar with the process"

---

## Token Optimization — Mandatory Rules

When evaluating a job posting, FOLLOW THESE RULES:

### WebSearch Restraint
- Maximum 2 web searches per evaluation (NOT 3-4)
- For known major companies (in tracked_companies), trust the notes 
  field — skip "is this real company" verification searches
- For salary, ONE search is enough — use profile.yml ranges as baseline
- NEVER fetch full Glassdoor/Levels.fyi pages — use search snippets only
- Skip update-system check unless user explicitly asks

### Report Brevity
- Block tables: 5-6 rows max (not 10-12)
- Block F (Interview): 4-5 stories max (not 6-7)
- Skip "Top 5 LinkedIn changes" section unless user asks
- TL;DR style — tight, actionable, not exhaustive

### CV Generation Efficiency
- Reuse the previous CV HTML in output/ as a base template — only modify 
  sections that need it (Summary, Competencies, top bullets, Project order)
- Sections that are static across CVs (Education, bottom Skills block, 
  junior Birinci entries) — copy verbatim, don't regenerate

Target: <12K total tokens per evaluation including HTML CV.

---

## Known Company Heuristic

Companies in portals.yml's tracked_companies list are PRE-VERIFIED.
For these companies, SKIP these searches:
- Company existence verification
- Layoff news (unless JD has explicit red flags)  
- Stability searches
...
Peak Games, Bitaksi, Yildiz Holding, Arabam.com, Shipday.

ONLY run salary search if salary data is genuinely uncertain. 
For first-tier Turkey fintech/e-commerce: use profile.yml's compensation 
section (100-200K TL Senior DS) without further searching.
